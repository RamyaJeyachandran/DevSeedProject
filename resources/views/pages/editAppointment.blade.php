...
@include('layouts.sideMenu')<!-- BEGIN: Content -->
<div class="content"> @include('layouts.topBar') 
    ...
    <input id="txtHospital" name="hospitalId" value="{{ session('hospitalId') }}" type="hidden" class="form-control">
        ...
        <input id="txtAppointmentId" name="appointmentId" value="{{$appointmentDetails->id}}" type="hidden" class="form-control">
        <input id="txtPatient" name="patientId" value="{{$appointmentDetails->patientId}}" type="hidden" class="form-control">
        <input id="txtType" name="type" value="{{$appointmentDetails->type}}" type="hidden" class="form-control">
    <div class="intro-y box p-5 mt-5"> <div class="grid grid-cols-12 gap-4 gap-y-5 mt-5"> <div
        ...
        *</span></label>
        <input id="txtAppointmentDate" name="appointmentDate" value="{{$appointmentDetails->appointmentDate}}" type="text" class="datepicker form-control "
            data-single-mode="true" required> </div> <div class="intro-y col-span-12 sm:col-span-6">
        <label for="txtAppointmentTime" class="form-label">Appointment Time <span class="text-danger mt-2">
        *</span></label>
        <input id="txtAppointmentTime" name="appointmentTime" value="{{$appointmentDetails->appointmentTime}}" type="time" class="form-control" required>
        </div>
        <div class="intro-y col-span-12 sm:col-span-6">
        <label for="ddlDepartment" class="form-label">Department</label>
        <select id="ddlDepartment"  value="{{$appointmentDetails->departmentId}}" name="departmentId" class="form-select">
                                    <option value='0'>Select Department</option>
                                @foreach ($appointmentDetails->departmentList as $department)
                                    <option value="{{ $department->id }}" {{ ( $department->id == $appointmentDetails->departmentId) ? 'selected' : '' }}> 
                                        {{ $department->name }} 
                                    </option>
                                @endforeach    
                                </select>
        ...
        <select id="ddlDoctor"  value="{{$appointmentDetails->doctorId}}" name="doctorId" class="form-select" required>
                <option value='0'>Select Doctor</option>
                @foreach ($appointmentDetails->doctorList as $doctor)
                        <option value="{{ $doctor->id }}" {{ ( $doctor->id == $appointmentDetails->doctorId) ? 'selected' : '' }}> 
                            {{ $doctor->name }} 
                        </option>
                @endforeach    
        </select>
        ...
        <label for="txtReason" class="form-label">Reason For Vist <span class="text-danger mt-2">
                *</span></label>
                <textarea id="txtReason" name="reason" value="{{$appointmentDetails->reason}}" class="form-control" minlength="10">{{$appointmentDetails->reason}}</textarea>
    </div>
    <div class="intro-y col-span-12 sm:col-span-6">
    <label for="ddlStatus" class="form-label">Status</label>
        <select id="ddlStatus"  value="{{$appointmentDetails->status}}" name="status" class="form-select" required>
                <option value='0'>Select Status</option>
                @foreach ($appointmentDetails->statusList as $StatusDetails)
                        <option value="{{ $StatusDetails->name }}" {{ ( $StatusDetails->name == $appointmentDetails->status) ? 'selected' : '' }}> 
                            {{ $StatusDetails->name }} 
                        </option>
                @endforeach    
        </select>
...
<button id="btnUpdateAppointment" type=submit class="btn btn-danger shadow-md mr-2"><i data-lucide="save"
                                class="w-4 h-4 mr-2"></i>Update</button>
                        </div>
</div>
</div>
    ...
    <div class="flex flex-col lg:flex-row border-b border-slate-200/60 dark:border-darkmode-400 pb-5 -mx-5">
        <div class="flex flex-1 px-5 items-center justify-center lg:justify-start">
            ...
            <div class="ml-5">
                ...
                <div id="lblHcNo" class="text-info">Code No : <span>{{ $appointmentDetails->hcNo}}</span></div>
                <div id="lblPhoneNo" class="truncate sm:whitespace-normal flex items-center"><i data-lucide="phone"
                        class="w-4 h-4 mr-2"></i><span>{{ $appointmentDetails->phoneNo}}</span> </div>
                <div id=lblEmail class="truncate sm:whitespace-normal flex items-center"><i data-lucide="mail"
                        class="w-4 h-4 mr-2"></i><span>{{ $appointmentDetails->email}}</span></div>
                <div id=lblAddress class="truncate sm:whitespace-normal flex items-center"><i data-lucide="home"
                        class="w-4 h-4 mr-2"></i><span>{{ $appointmentDetails->address}}</span></div>
            </div>
        </div>
        <div
            ...
            <div class="flex flex-col justify-center items-center lg:items-start mt-4">
                ...
                <div id="lblMartialStatus" class="truncate sm:whitespace-normal flex items-center mt-3"><b>Martial
                        Status
...
<div id="divSuccessEditAppointment" data-tw-backdrop="static" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="px-5 pb-8 text-center"> <button id="btnAppointmentRedirect" type="button" data-tw-dismiss="modal"
